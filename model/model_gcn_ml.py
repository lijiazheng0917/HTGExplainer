...
class HTGNNLayer(nn.Module):
    ...
    def forward(self, graph: dgl.DGLGraph, node_features: dict, edge_weight=None):
        """
        ...
        for stype, etype, dtype in graph.canonical_etypes:
            k = graph[stype, etype, dtype].number_of_edges()
        ...
        for stype, etype, dtype in graph.canonical_etypes:
            ...
            if edge_weight==None:
                ...
                # print(edge_weight[dic[etype][0]:dic[etype][1]-1].shape[0])
                # print(rel_graph.number_of_edges())
                

                dst_feat = self.intra_rel_agg[etype](rel_graph, (node_features[stype][ttype], node_features[dtype][ttype]),edge_weight=edge_weight[dic[etype][0]:dic[etype][1]])
        ...
        for ttype in intra_features.keys():
            for ntype in graph.ntypes:
                ...
                # print(types_features.dim())
                if types_features.dim() == 2: 
                    types_features = types_features.T.unsqueeze(0)
                # print(types_features.shape)
...
class HTGNN(nn.Module):
    ...
    def forward(self, graph: dgl.DGLGraph, edge_weight=None):
        """
        ...
        for i in range(self.n_layers):
            inp_feat = self.gnn_layers[i](graph, inp_feat, edge_weight)
        ...
        #     for t in predict_type:
        #         out_feat[t] = sum([inp_feat[t][ttype] for ttype in self.timeframe])
        # else:
        #     out_feat = sum([inp_feat[predict_type][ttype] for ttype in self.timeframe])
