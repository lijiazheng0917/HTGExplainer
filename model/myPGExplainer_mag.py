...
# writer = SummaryWriter('/home/jiazhengli/xdgnn/HTGNN/output1/mag_es_pg')
...
class PGExplainer():
    ...
    def __init__(self, model_to_explain, G_train, G_train_label, G_val, G_val_label, G_test, G_test_label, time_win, node_emb, device,
        ...
        if self.explain_method == 'our':
            ...
            self.writer = SummaryWriter('output1/mag_pg')
    ...
    def evaluate(self, exp, G_val, G_val_label, t):
        ...
        for i in range(len(G_val)):
            ...
            for n in edge_list:
                ...
                #input_expl = self._create_explainer_input(sg, embed, src, dst, self.het_emb, self.etype_dic, self.tem_emb).unsqueeze(0).to('cuda')

                #sampling_weights = self.explainer_model(input_expl)
                sampling_weights = exp(sg, embed, src, dst, self.etype_dic, self.device, self.explain_method,'mag')
    ...
    def train(self):
        ...
        model_out_path = 'output1/explainer_mag_seed2024'
        # early_stopping
        early_stopping = EarlyStopping(patience=self.es_epoch, verbose=True, path=f'{model_out_path}/checkpoint_mag_{self.explain_method}_{self.te}_{self.he}.pt')
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for i in range(len(self.G_train)):
                ...
                for j in range(0, 10):
                    ...
                    for n in training_list:
                        ...
                        #sampling_weights = self.explainer_model(input_expl)
                        sampling_weights = self.exp(sg, embed, src, dst, self.etype_dic, self.device, self.explain_method,'mag')
            ...
            #     torch.save(self.explainer_model.state_dict(), f'{model_out_path}/checkpoint_mag_{self.explain_method}_{self.te}_{self.he}.pt')
            #     break
        ...
        if self.test_only:
            for test_ in [0]:
                ...
                # self.explainer_model.load_state_dict(torch.load(f'{model_out_path}/checkpoint_mag_{self.explain_method}_{self.te}_{self.he}.pt'))
                # model_out_path = f'output1/explainers_{test_}/mag'
                model_out_path = f'output1/explainer_mag_seed2024'
                self.exp.load_state_dict(torch.load(f'{model_out_path}/checkpoint_mag_{self.explain_method}_{self.te}_{self.he}.pt'))
                self.exp.eval()
                # self.het_emb.eval()
                all_pred = []
                target_list = []
