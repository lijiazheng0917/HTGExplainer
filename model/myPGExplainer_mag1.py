...
class PGExplainer(BaseExplainer):
    ...
    """
    def __init__(self, model_to_explain, G, G_label, G2, task, time_win, epochs=30, lr=0.005, temp=(5.0, 2.0), reg_coefs=(0.0002, 1),sample_bias=0):
        super().__init__()
    ...
    def train(self, indices = None):
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for i in range(batchs):
                ...
                mloss = 0
                sloss = 0
                # loss_ = torch.FloatTensor([0]).detach().to('cuda')
                # cce_loss = torch.FloatTensor([0]).detach().to('cuda')
                # size_loss = torch.FloatTensor([0]).detach().to('cuda')
                # mask_ent_loss = torch.FloatTensor([0]).detach().to('cuda')
                ...
                for n in range(lo, hi):
                    ...
                    n = int(n)
                    # print(n)
                    src, dst = self.G_label[0].edges()[0][n], self.G_label[0].edges()[1][n]

                    src_sg, _ = dgl.khop_in_subgraph(self.G, {'author': src}, k=2, store_ids=True)
                    dst_sg, _ = dgl.khop_in_subgraph(self.G, {'author': dst}, k=2, store_ids=True)

                    src_input_expl = self._create_explainer_input(src_sg, embed, n).unsqueeze(0).to('cuda')
                    dst_input_expl = self._create_explainer_input(dst_sg, embed, n).unsqueeze(0).to('cuda')

                    src_sampling_weights = self.explainer_model(src_input_expl)
                    dst_sampling_weights = self.explainer_model(dst_input_expl)

                    src_mask = self._sample_graph(src_sampling_weights, t, bias=self.sample_bias).squeeze().to('cuda')
                    dst_mask = self._sample_graph(dst_sampling_weights, t, bias=self.sample_bias).squeeze().to('cuda')

                    src_h_m = self.model_to_explain[0](src_sg.to('cuda'),'author',edge_weight=src_mask)
                    dst_h_m = self.model_to_explain[0](dst_sg.to('cuda'),'author',edge_weight=dst_mask)

                    src_h = src_h_m[torch.where(src_sg.ndata[dgl.NID]['author'] == src)]
                    dst_h = dst_h_m[torch.where(dst_sg.ndata[dgl.NID]['author'] == dst)]
                    ...
                    size_loss = torch.sum(src_mask) * size_reg + torch.sum(dst_mask) * size_reg
                    mask_ent_reg1 = -src_mask * torch.log(src_mask) - (1 - src_mask) * torch.log(1 - src_mask)
