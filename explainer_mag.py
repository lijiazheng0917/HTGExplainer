...
from model.HTGNN import HTGNN, LinkPredictor
from utils.data import load_MAG_data
from model.HTGExplainer_mag import HTGExplainer
...
def main():
    ...
    parser.add_argument('--tw', default=5, type=int, help='time window, follow htgnn setting')
    ...
    train_feats, train_labels, val_feats, val_labels, test_feats, test_labels = load_MAG_data(glist, args.tw, device)

    # load the to-be-explained model -- HTGNN
    graph_atom = test_feats[0]
    htgnn = HTGNN(graph=graph_atom, n_inp=128, n_hid=32, n_layers=2, n_heads=1, time_window=args.tw, norm=True, device=device)
    predictor = LinkPredictor(n_inp=32, n_classes=1)
    model = nn.Sequential(htgnn, predictor).to(device)
    model.load_state_dict(torch.load('HTGNN/checkpoint_HTGNN_mag.pt'))
